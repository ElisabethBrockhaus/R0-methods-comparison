...
GTd <- R0::generation.time("gamma", c(20/7, 15/7))
...
for(i in 1:1){ 
  ...
  for(j in 1:ncol(simulations$sims)){ 
    ...
    # fit & store results
    results_i[[j]] <- fit_R0_seq(data=sim_ij, mean_GT=20/7, sd_GT=15/7, GTd=GTd, GT_week=3)
    print(j)
  }
...
metrics <- performance_metrics(trueR0=simulations$pars, estimates=Sim_results[[1]], max_weeks=15, min_peak=15)
